...
def fetch_details(request):
    ...
    if uid:
        try:
            ...
            if driver_id:
                consignments = consignments.filter(driver_id__icontains=driver_id)
    ...
    if request.method == 'POST':
        ...
        if sender_name and from_date and to_date:
            try:
                ...
                # Convert from_date and to_date to string format if needed
                from_date_str = from_date.strftime('%Y-%m-%d')
                to_date_str = to_date_end.strftime('%Y-%m-%d')

                logger.info(f"Formatted dates: from_date_str: {from_date_str}, to_date_str: {to_date_str}")

                # Filter accounts by sender_name and Date within the specified range
                accounts = Account.objects.filter(
                    sender_name=sender_name, Branch=branch
                ).filter(
                    Date__gte=from_date_str,
                    Date__lt=to_date_str  # Use __lt for the end date to include the entire day
                ).values(
        ...
        if uid:
            try:
                ...
                Expenses.objects.create(
                    ...
                    GstPer=request.POST.get('gst') or None,  # Accept null
                    CGSTPer = request.POST.get('cgst') or None,  # Accept null
                    SGSTPer = request.POST.get('sgst') or None , # Accept null
...
def adminView_Consignment(request):
    userdata = []  # Initialize consignee as an empty list

    if request.method == 'POST':
        branch = request.POST.get('t2')
        print(f"Branch: {branch}")  # Debugging: Print the branch name
        userdata = AddConsignment.objects.filter(branch=branch)
        print(f"Consignee: {userdata}")  # Debugging: Print the consignee queryset
    return render(request,'adminView_Consignment.html',{'userdata':userdata})
...
def admininvoiceConsignment(request, track_id):
    ...
    # Pass the consignment, the entire list of items, and the branch details to the template
